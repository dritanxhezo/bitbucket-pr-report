...
public class DefaultDemoControllerTest {
    ...
    public void shouldLoadPullRequests() throws IOException {
        ...
        PullRequestModel[] expectedPullRequestModels = new PullRequestModel[] {
                new PullRequestModel(1, "description 1", "open", dt1, "ngeor"),
                new PullRequestModel(2, "description 2", "rejected", dt2, "test", "reviewer1", "reviewer2")
        };
        ...
        when(client.execute(new PullRequestsRequest("currentUser", "repo"), PullRequestsResponse.class))
                .thenReturn(new PullRequestsResponse(
                        new PullRequestResponse(1, "description 1", "open",  dt1, new Author("ngeor", "Nikolaos")),
                        new PullRequestResponse(2, "description 2", "rejected", dt2, new Author("test", "Test User")) // participants not included
                ));
        when(client.execute(new PullRequestRequest("currentUser", "repo", 1), PullRequestResponse.class))
                .thenReturn(new PullRequestResponse(1, "description 1", "open", dt1, new Author("ngeor", "Nikolaos")));
        when(client.execute(new PullRequestRequest("currentUser", "repo", 2), PullRequestResponse.class))
                .thenReturn(new PullRequestResponse(2, "description 2", "rejected", dt2, new Author("test", "Test User"),
