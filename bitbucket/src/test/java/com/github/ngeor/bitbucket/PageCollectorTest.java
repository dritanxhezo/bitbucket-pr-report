...
class PageCollectorTest {
    @Test
    void collect() {
        PaginatedRepositories firstPage = (PaginatedRepositories) new PaginatedRepositories()
                                              .addValuesItem(new Repository().uuid("abc"))
                                              .next("http://page-2");
        PaginatedRepositories secondPage =
            (PaginatedRepositories) new PaginatedRepositories().addValuesItem(new Repository().uuid("def")).next("");

        // act
        Iterable<PaginatedRepositories> paginatedRepositories = PageCollector.collectAll(new Paginator<>() {
            @Override
