...
public class BitbucketClient {
    ...
    public Stream<Repository> getAllRepositories() {
        var iterable = PageCollector.collectAll(new Paginator<PaginatedRepositories>() {
            @Override
            ...
            @Override
            public PaginatedRepositories next(PaginatedRepositories previousPage) {
        ...
        return StreamSupport.stream(iterable.spliterator(), false).flatMap(r -> r.getValues().stream());
    }
    ...
    public Stream<Pipeline> getAllPipelines(String slug) {
        var iterable = PageCollector.collectAll(new Paginator<PaginatedPipelines>() {
            @Override
            public PaginatedPipelines first() {
                String url = baseUrl + "repositories/" + owner + "/" + slug + "/pipelines/";
            ...
            public PaginatedPipelines next(PaginatedPipelines previousPage) {
                ...
                String nextUrl =
                    baseUrl
                    + String.format("repositories/%s/%s/pipelines/?page=%d", owner, slug, previousPage.getPage() + 1);
     ...
     */
    public Stream<PullRequest> getAllPullRequests(String slug) {
        var iterable = PageCollector.collectAll(new Paginator<PaginatedPullRequests>() {
            @Override
            public PaginatedPullRequests first() {
                String url = baseUrl + "repositories/" + owner + "/" + slug + "/pullrequests?state=MERGED";
